    ...
    <v-content class="mt-12">
      ...
      <v-row class="mt-12">
        <v-col class="mt-12 text-justify" cols="10" offset-lg="3" lg="8" offset="1">
          <router-link :to="{ path: '/record/' + this.$route.params.id, params: { id: this.$route.params.id }}"
                       target="_blank">
      ...
      <v-row>
        ...
        <v-col cols="10" lg="8" sm="10" class="mt-4">
          <ckeditor :key="editorConfig.editorKey" v-model="species_text" :config="editorConfig" :editor-url="editorConfig.editorUrl"></ckeditor>
...
export default {
  ...
  data() {
    return {
      species_id: this.$store.state.speciesText.species_id,
      // Clone species_text for local manipulation
      species_text: _.cloneDeep(this.$store.state.speciesText.species_text),
      editorConfig: {
  ...
  beforeCreate() {
    this.$store.dispatch('speciesText/getSpeciesText');
  },
  methods: {
    ...mapActions('speciesText', [
      'setUpdatedText',
    ]),
    ...
    handleSave(event) {
      ...
      const textObj = JSON.stringify(that.species_text.replace(/^<div[^>]*>|<\/div>$/gm, '').trim());
      ...
      axios.put(`/admin/edit-species/${that.species_id}`, textObj, config).then((response) => {
    ...
    forceRerender() {
      ...
      this.species_text = _.cloneDeep(this.$store.state.speciesText.species_text);
      this.loading = false;
      this.error = false;
      this.success = null;
    },
  ...
  computed: {
    storeText() {
      return _.cloneDeep(this.$store.state.speciesText.species_text);
    },
    getLoadState() {
      return this.$store.getters['speciesText/loadState'];
    },
    getSciName() {
      return this.$store.getters['speciesText/species_sci_name'];
    },
  },
  watch: {
    storeText(newValue) {
      this.species_text = newValue;
    },
